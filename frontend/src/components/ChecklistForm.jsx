  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      const result = await syncManager.submitWithOfflineSupport({
        ...
        method: 'POST',
        payload: formMapped,
        description: `Inspeção SDAI: Loja ${formData.loja || ''} (${formData.codigoLoja || ''})`,
        tenant,
        type: 'checklist_sdai',
      });
      ...
      if (result.success) {
        if (result.offline) {
          setSubmitStatus({
            type: 'success',
            message: '💾 Modo Offline: Relatório salvo com segurança no dispositivo! Ele será enviado e processado automaticamente quando a conexão retornar.',
          });
        } else {
          setSubmitStatus({
            type: 'success',
      ...
      setSubmitStatus({
        type: 'error',
        message: 'Erro inesperado ao salvar relatório.',
      });
