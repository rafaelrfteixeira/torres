 ...
 *
 * Replica a funcionalidade do SDAI.html standalone:
 * - KPIs (Total de Lojas, Inspecionadas, Parciais, Com Defeito, Cobertura)
 * - Filtros Operacionais (Status, Segmento, Busca)
 * - Tabela agrupada por loja com histórico expansível
 *
 * Os dados vêm diretamente do backend via Graph API (sem N8N).
 */
...
function getStatusBadge(status) {
  ...
  const c = config[status] || config['Sem Detecção'];
...
export default function DashboardSDAI({ user, shoppingsMetadata = [] }) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_URL}/checklists/report?tenant=${tenant}&checklist_type=sdai`,
          { credentials: 'include' }
        );
  ...
  // ============================================
  // Process raw inspection data into grouped store data
  // ============================================
  function processarDados(registrosBrutos, totalLojas) {
    ...
    registrosBrutos.forEach((item) => {
      ...
      if (item.status_com_defeito === 'Sim') statusItem = 'Com Defeito';
        ...
        statusItem = 'Sem Detecção';
      else if (item.status_funcionando_normalmente === 'Sim')
        statusItem = 'Normal';

      // Inventário
      const disps = [];
      if (item.numero_DF > 0) disps.push(`${item.numero_DF} DF`);
      if (item.numero_DT > 0) disps.push(`${item.numero_DT} DT`);
      if (item.numero_AM > 0) disps.push(`${item.numero_AM} AM`);
      if (item.numero_DG > 0) disps.push(`${item.numero_DG} DG`);
      if (item.numero_modulos > 0) disps.push(`${item.numero_modulos} Mod`);
      if (item.numero_sirenes > 0) disps.push(`${item.numero_sirenes} Sir`);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-lg sm:text-2xl font-bold tracking-tight text-white mt-1">
                Inspeção de SDAI — {currentShopping.name}
              </h1>
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Sem Detecção">Sem Detecção</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: Madero, BAR CHEF, 3009..."
              ...
              <div className="px-4 sm:px-6 py-4 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 bg-slate-50">
                <div className="flex items-center gap-2">
                  <h2 className="text-sm font-bold text-slate-800 tracking-wide uppercase">
                    Portfólio de Lojas e Histórico de Visitas
                  </h2>
...
function LojaRow({ loja, index, isExpanded, onToggle, tenant, onToast }) {
  const [loadingPdf, setLoadingPdf] = useState(null); // item id being loaded
  const [loadingResend, setLoadingResend] = useState(null); // item id being resent
  ...
  const handleViewPdf = async (e, itemId) => {
    ...
    try {
      const url = `${API_URL}/checklists/${itemId}/pdf?tenant=${tenant}&checklist_type=sdai`;
  ...
  const handleResendPdf = async (e, itemId) => {
    ...
    try {
      const url = `${API_URL}/checklists/${itemId}/resend?tenant=${tenant}&checklist_type=sdai`;
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-100 border-b-2 border-slate-300 text-[11px] text-slate-400 uppercase tracking-wider font-bold">
                    <th className="py-3.5 px-4 border-r border-slate-200">
                      Data
                    </th>
                    <th className="py-3.5 px-4 border-r border-slate-200">
                ...
                <tbody>
                  {loja.inspecoes.map((visit, vIdx) => (
                    ...
                    >
                      ...
                      <td
                        className="py-3.5 px-4 text-slate-900 font-medium max-w-xs break-words border-r border-slate-200"
                        title={visit.observacoes}
                      >
                        {visit.observacoes}
                      </td>
