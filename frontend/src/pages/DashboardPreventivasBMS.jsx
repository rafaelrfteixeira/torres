...
export default function DashboardPreventivasBMS({ user, shoppingsMetadata = [] }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/preventivas/dashboard-status?tenant=${tenant}&mes=${mesSelecionado}&ano=${anoSelecionado}&sistema=bms`,
        { credentials: 'include' }
      );

      if (!response.ok) {
        const text = await response.text();
        throw new Error(`HTTP ${response.status}: ${text || 'Falha ao buscar dados do dashboard BMS.'}`);
      }
      ...
      if (result.success && result.data) {
        ...
        throw new Error(result.message || 'Erro ao carregar dados de preventivas BMS.');
      }
    } catch (err) {
      console.error('❌ Erro no DashboardPreventivasBMS:', err);
  ...
  const dispositivosFiltrados = useMemo(() => {
    return dispositivosDoMes.filter((d) => {
      ...
      // 3. Busca livre (TAG, Descrição, Pavimento, Tipo)
      let bateBusca = true;
      if (buscaTag.trim()) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-semibold tracking-wider text-blue-200 uppercase flex items-center gap-1">
                ...
                TorresCx — Sistemas de Automação Predial (BMS)
              </span>
              <h1 className="text-lg sm:text-2xl font-bold tracking-tight text-white mt-1">
                Gestão de Preventivas e Cronograma BMS — {currentShopping.name}
              </h1>
            </div>
          </div>
      ...
      <div className="p-4 sm:p-6 max-w-[1600px] w-full mx-auto space-y-6">
          ...
          <div className="text-center py-12 bg-white rounded-xl border border-slate-200 shadow-sm">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-blue-700 text-sm font-semibold">
                Carregando matriz mestra BMS e cruzando histórico de preventivas...
              </p>
            </div>
          </div>
        )}
          ...
          <div className="text-center py-8 px-4 bg-white rounded-xl border border-red-200 shadow-sm max-w-xl mx-auto">
            <p className="text-red-600 text-sm font-semibold mb-2">
              {error.includes('401') || error.includes('Acesso negado') || error.includes('login')
                ...
                : `❌ Erro ao carregar dashboard BMS: ${error}`}
            </p>
            {error.includes('401') || error.includes('Acesso negado') || error.includes('login') ? (
              <a
        ...
        {!isLoading && !error && (
          <>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xs font-extrabold uppercase tracking-wider text-slate-700">
                  Panorama Geral de Preventivas BMS — Base de Dados Completa 2026
                </h2>
              </div>
            ...
            {/* 3. BLOCO DE FILTROS OPERACIONAIS DE BMS */}
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4">
              <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
                ...
                <h2 className="text-xs font-bold uppercase tracking-wider text-slate-700">
                  Filtros Avançados de Cronograma BMS
                </h2>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                {/* 3. Filtrar por Descrição / TAG */}
                <div>
                  <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">
                    Filtrar por TAG ou Descrição
                  </label>
                  <input
                    type="text"
                    value={buscaTag}
                    onChange={(e) => setBuscaTag(e.target.value)}
                    placeholder="Ex: 3019, SPOLETO, Medição..."
            ...
            {/* 4. TABELA PRINCIPAL: RASTREAMENTO CRONOLÓGICO DE ATIVOS BMS */}
              ...
              <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                <div>
                  <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider">
                    Detalhamento Técnico das Rotinas do Mês — BMS
                  </h2>
                  ...
                  <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                    {dispositivosFiltrados.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="p-8 text-center text-slate-400">
                          {dispositivosDoMes.length === 0
                            ? 'Nenhuma preventiva BMS programada para esta competência.'
                      ...
                      dispositivosFiltrados.map((disp) => {
                        ...
                        const hasValidTag = disp.tag && disp.tag !== disp.descricao && !disp.tag.startsWith('TAG-N/A') && !disp.tag.startsWith('TAG-');
                          ...
                          >
                            ...
                            <td className="p-4">
                              <div className="font-bold text-slate-900 text-sm">
                                {hasValidTag ? `${disp.tag} - ${disp.descricao}` : (disp.descricao || disp.tag)}
                              </div>
                              {disp.pavimento && (
                                <div className="text-xs text-slate-400 font-medium mt-0.5">{disp.pavimento}</div>
                              )}
                            </td>
                            <td className="p-4">
                              <span className="text-xs bg-slate-100 px-2.5 py-1 rounded border border-slate-200 font-bold text-slate-700 inline-block">
                                {disp.tipo || 'Geral'}
                              </span>
                            </td>
      ...
      {/* 5. DRAWER DE HISTÓRICO DO ATIVO BMS SELECIONADO */}
      {ativoSelecionado && (
            ...
            <div className="p-5 bg-slate-900 text-white flex justify-between items-center border-l-4 border-blue-500">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-wider text-blue-400">
                  Ficha Técnica do Ativo BMS
                </span>
                <h3 className="text-base font-bold mt-0.5">
                  {ativoSelecionado.tag && ativoSelecionado.tag !== ativoSelecionado.descricao
                    ? `${ativoSelecionado.tag} — ${ativoSelecionado.descricao}`
                    : ativoSelecionado.descricao}
                </h3>
              </div>
              <button
            ...
            <div className="p-6 flex-1 overflow-y-auto space-y-6">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3 text-sm">
                ...
                <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-200">
                  {ativoSelecionado.tag && (
                    <div>
                      <span className="text-xs font-semibold text-slate-400 uppercase">TAG</span>
                      <p className="font-mono font-bold text-slate-800">{ativoSelecionado.tag}</p>
                    </div>
                  )}
                  <div>
              ...
              {ativoSelecionado.realizado && (() => {
                  ...
                  return (
                    <div className="border border-red-200 bg-red-50/70 rounded-xl p-4 space-y-3">
                      <div className="flex items-center gap-2 text-red-700 font-bold text-xs uppercase tracking-wide">
                        ...
                        Dispositivo Sem Acesso
                      </div>
                      <p className="text-xs text-red-600">
                        O técnico registrou que o dispositivo não pôde ser inspecionado por falta de acesso físico ou técnico no local.
                      </p>
                    </div>
                  );
                }
                ...
                if (hasRealChecklist) {
                  return (
                    <div className="border border-slate-200 rounded-xl p-4 space-y-3">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-slate-600 border-b border-slate-100 pb-2">
                        Checklist Operacional Realizado
                      </h4>
                      <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                        {ativoSelecionado.logChecklist.map((item, idx) => {
                          const isOk = item.status === 'sim' || item.status === 'OK' || item.status === 'conforme';
                          return (
                            <div
                              key={idx}
                              className={`p-2.5 rounded-lg border text-xs flex items-center justify-between gap-2 ${
                                isOk ? 'bg-emerald-50/50 border-emerald-200 text-emerald-900' : 'bg-rose-50/50 border-rose-200 text-rose-900'
                              }`}
                            >
                              <span className="font-medium">{item.atividade || item.item}</span>
                              <span
              ...
              {/* Fotos Evidência */}
              {(ativoSelecionado.foto1Url || ativoSelecionado.foto2Url) && (
                <div className="border border-slate-200 rounded-xl p-4 space-y-3">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-slate-600 border-b border-slate-100 pb-2">
                    Evidências Fotográficas
                  </h4>
                  <div className="grid grid-cols-2 gap-3">
                    {ativoSelecionado.foto1Url && (
                      <div className="space-y-1">
                        <span className="text-[10px] font-semibold text-slate-400 block">Foto 1</span>
                        <img
                          src={ativoSelecionado.foto1Url}
                          alt="Evidência 1"
                          className="w-full h-32 object-cover rounded-lg border border-slate-200"
                        />
                      </div>
                    )}
                    {ativoSelecionado.foto2Url && (
                      <div className="space-y-1">
                        <span className="text-[10px] font-semibold text-slate-400 block">Foto 2</span>
                        <img
                          src={ativoSelecionado.foto2Url}
                          alt="Evidência 2"
                          className="w-full h-32 object-cover rounded-lg border border-slate-200"
                        />
                      </div>
